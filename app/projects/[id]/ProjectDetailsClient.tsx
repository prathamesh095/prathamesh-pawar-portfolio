...
// Define the Project type (same as in page.tsx)
interface Project {
  id: string
  title: string
  tagline: string
...
export default function ProjectDetailsClient({ project }: ProjectDetailsClientProps): JSX.Element {
            ...
            <Card className="bg-background/95 backdrop-blur-md border border-primary/30 shadow-xl shadow-primary/20 animate-glow">
              <CardContent className="p-6">
                ...
                <nav className="space-y-2">
                  {navigationItems.map((item) => {
                    const Icon = iconMap[item.icon.name] || Target // Fallback to Target if icon not found
                    return (
                      <button
                        key={item.id}
                        onClick={() => scrollToSection(item.id)}
          ...
          <Card className="shadow-2xl shadow-primary/20 bg-gradient-to-br from-background/95 to-primary/10 backdrop-blur-md rounded-3xl overflow-hidden border border-primary/30 animate-glow">
            <CardContent className="p-8">
              ...
              <div className="space-y-6">
                {project.process.map((step: Project["process"][number], index: number) => {
                  const Icon = iconMap[step.icon] || Target // Fallback to Target if icon not found
                  return (
                    <motion.div
                      key={index}
